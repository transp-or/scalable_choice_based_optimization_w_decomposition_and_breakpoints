...
function compute_obj_value_priority_queue_scaled(x, S, h, phi, theta, caps, prio_queue)
    (NS,C) = size(theta)
    D = C-1
    idx_d = sortperm(x) # identify ordering of prices
    push!(idx_d, C) # idx_d[D+1] = D+1
...
function compute_obj_value_with_forced_capacities(x, S, h, phi, theta, caps; inv=false)
    (NS,C) = size(theta)
    D = C-1
    idx_d = sortperm(x) # identify ordering of prices
    push!(idx_d, C) # idx_d[D+1] = D+1
...
function optimize_parameters_cmaes_caps(
    a::Vector{Float64}, 
    b::Vector{Float64},
    S, D,
    exo_utility_, 
    ...
    for iter in 1:max_iter
        ...
        f_values = [bounded_objective_function_new(collect(x), S, D, exo_utility_, endo_coef_, caps, prio_queue, lb, ub) for x in eachrow(X)]
        ...
        ps_new = (1 - c1) * ps + sqrt(c1 * (2 - c1) * λ) * (m_new - m) / σ
		m_old = m
        m = m_new
...
"""
Bounded objective wrapper.
Adds large penalties if θ violates bounds.
"""
function bounded_objective_function_new(
    ...
    # Compute objective
    obj_value = obj_value_caps_fun(θ, S, zeros(D, D+1), exo_utility_, endo_coef_, caps, prio_queue)
...
function run_algo(N, S, J_PSP, J_PUP)
	...
	# GA settings
	pop_size = 200       # Population size (could be up to 200)
	generations = 500    # Number of generations to run (could be up to 1000)
	...
	best_prices, best_obj = optimize_parameters_cmaes_caps(a, b, S, D, exo_utility_, endo_coef_, caps, prio_queue)
    end).time
