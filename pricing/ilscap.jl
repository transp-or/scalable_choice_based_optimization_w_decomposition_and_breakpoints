...
function run_algo(N, S, J_PSP, J_PUP)
	...
	start_obj, start_prices, start_iter = polytime_heuristic(exo_utility, endo_coef, start_prices, a, b)
    best_obj, best_prices, iter = polytime_cap_heuristic(exo_utility_, endo_coef_, S, start_prices, a, b, caps, prio_queue)
    best_obj, best_prices, iterations = extended_line_search_cap(exo_utility_, endo_coef_, S, a, b, best_prices, best_obj, caps, prio_queue)
    end).time
